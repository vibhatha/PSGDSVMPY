...
class AllReduceAdvancedExample:
    ...
    def execute(self):
        ...
        #perform reduction based on sum and maximum
        self.comms.allreduce(input=input, output=output_max, op=self.comms.mpi.MAX, dtype=self.comms.mpi.INT, root=0)
        self.comms.allreduce(input=input, output=output_sum, op=self.comms.mpi.SUM, dtype=self.comms.mpi.INT, root=0)
...
ex1 = AllReduceExample().execute
ex2 = AllReduceAdvancedExample().execute
