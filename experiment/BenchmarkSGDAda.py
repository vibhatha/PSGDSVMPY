...
class BenchmarkSGDAda:

    def __init__(self, exp_name ,training_file, testing_file, features=1, alpha=0.01,
                 epochs=100, labelfix=False, split=False, randomize=False, auto=False, bulk=False, C=1, gamma=1):
    ...
    def train(self):
        ...
        self.w = self.svm.ada_sgd_train(X=self.x_training, y=self.y_training, alpha=self.alpha, epochs=self.epochs, w_init=self.w_init)
        self.training_time += (time.time())
        return self.svm
    ...
    def train_alpha(self,alpha=0.01):
        ...
        self.training_time -= (time.time())
        Print.Print.console('Manual Alpha Mode')
        self.w = self.svm.ada_sgd_train(X=self.x_training, y=self.y_training, alpha=alpha, epochs=self.epochs, w_init=self.w_init)
        self.training_time += (time.time())
        return self.svm

    def train_benchmark_light(self, w_init=[], x_train=[], y_train=[], x_test=[], y_test=[], log_file='', tolerance=0.001, indices_init=[], alpha=0.01):
        self.training_time = 0
        self.effective_epochs = 0
        self.cost = 0
        self.initial_cost = 0
        self.io_time = 0
        Print.Print.info1("Training with SGD + Ada ...")
        self.training_time = 0
        ...
        self.training_time -= (time.time())
        self.w, self.effective_epochs, self.cost, self.io_time, self.initial_cost = self.svm.ada_sgd_train_light(
            X=x_train, y=y_train, X_test=x_test, y_test=y_test, alpha=alpha,
        ...
        Print.Print.info1("Ada Training Completed !")
    ...
    def train_benchmark_heavy(self, w_init=[], x_train=[], y_train=[], x_test=[], y_test=[], log_file='', tolerance=0.001, indices_init=[]):
        ...
        self.training_time -= (time.time())
        self.w, self.effective_epochs, self.cost, self.io_time, self.initial_cost = self.svm.ada_sgd_train_light(
            X=x_train, y=y_train, X_test=x_test, y_test=y_test, alpha=self.alpha,
            epochs=self.epochs, w_init=w_init, indices_init=indices_init, log_file=log_file, tolerance=tolerance)
        self.training_time += (time.time())
        self.training_time = self.training_time - self.io_time
        Print.Print.info1("Ada Training Completed !")
    ...
    def stats(self):
        ...
        fp = open("logs/" + socket.gethostname()+"_"   + self.exp_name + "_ada_results.txt", "a")
    ...
    def advanced_stats(self, effective_epochs=0, accuracy=0, training_time=0, initial_cost=0, final_cost=0):
        Print.Print.result1(self.exp_name + "ADA Accuracy : " + str(self.acc) + "%")
        Print.Print.result1(self.exp_name + "ADA Time : " + str(self.training_time) + " s")
        fp = open("logs/benchmark/summary/" + socket.gethostname() + "_" + self.exp_name + "_ada_results.txt", "a")
        # fp.write("alpha : " + str(self.alpha) + ", epochs : " + str(self.epochs) + ", accuracy : " + str(self.acc) + "%" + ", time : " + str(self.training_time) + " s\n")
