...
class BenchmarkSGDAdam:
    ...
    def train(self, beta1=0.93, beta2=0.999):
        ...
        self.beta1 = beta1
        self.beta2 = beta2
        self.svm = SVM.SVM(trainPath=self.training_file, testPath=self.testing_file,X=self.x_training, y=self.y_training, alpha=self.alpha, n_features=self.n_features, randomize=self.randomize, bulk=self.bulk, split=self.split)
        self.w_init = self.svm.init_weights()
        self.training_time -= (time.time())
        Print.Print.console('ADAM SGD Training ...')
        self.w = self.svm.adam_sgd_train(X=self.x_training, y=self.y_training, X_testing=self.x_testing, y_testing=self.y_testing, alpha=self.alpha, epochs=self.epochs, w_init=self.w_init, beta1=beta1, beta2=beta2)
        self.training_time += (time.time())
        return self.svm

    def train_benchmark_light(self, w_init=[], x_train=[], y_train=[], beta1=0.93, beta2=0.999, x_test=[], y_test=[], log_file='',
                              tolerance=0.001, indices_init=[], alpha=0.01):
        self.training_time = 0
        ...
        self.w, self.effective_epochs, self.cost, self.io_time, self.initial_cost = self.svm.adam_sgd_train_light(
            X=x_train, y=y_train, X_test=x_test, y_test=y_test, alpha=alpha, beta1=beta1, beta2=beta2,
    ...
    def train_benchmark_heavy(self, w_init=[], x_train=[], y_train=[], beta1=0.93, beta2=0.999, x_test=[], y_test=[], log_file='',
                              tolerance=0.001, indices_init=[]):
        self.training_time = 0
        self.effective_epochs = 0
        self.cost = 0
        self.initial_cost = 0
        ...
        self.training_time -= (time.time())
        self.w, self.effective_epochs, self.cost, self.io_time, self.initial_cost = self.svm.adam_sgd_train_light(
            X=x_train, y=y_train, X_test=x_test, y_test=y_test, alpha=self.alpha, beta1=beta1, beta2=beta2,
            epochs=self.epochs, w_init=w_init, indices_init=indices_init, log_file=log_file, tolerance=tolerance)
        self.training_time += (time.time())
        self.training_time = self.training_time - self.io_time
        Print.Print.info1("Adam Training Completed !")
    ...
    def stats(self):
        ...
        fp = open("logs/" + socket.gethostname()+"_"   + self.exp_name + "_adam_results.txt", "a")
        #fp.write("alpha : " + str(self.alpha) + ", epochs : " + str(self.epochs) + ", accuracy : " + str(self.acc) + "%" + ", time : " + str(self.training_time) + " s\n")
        fp.write(str(self.alpha) + "," + str(self.beta1) + "," + str(self.beta2) + "," + str(self.epochs) + "," + str(self.acc) + "," + str(self.training_time) + "\n")
        fp.close()

    def advanced_stats(self, effective_epochs=0, accuracy=0, training_time=0, initial_cost=0, final_cost=0):
        Print.Print.result1(self.exp_name + "ADAM Accuracy : " + str(self.acc) + "%")
        Print.Print.result1(self.exp_name + "ADAM Time : " + str(self.training_time) + " s")
        fp = open("logs/benchmark/summary/" + socket.gethostname() + "_" + self.exp_name + "_adam_results.txt", "a")
        # fp.write("alpha : " + str(self.alpha) + ", epochs : " + str(self.epochs) + ", accuracy : " + str(self.acc) + "%" + ", time : " + str(self.training_time) + " s\n")
        fp.write(str(self.alpha) + "," + str(self.beta1) + "," + str(self.beta2) + str(self.epochs) +  "," + str(effective_epochs) + ","  + str(initial_cost) + "," + str(final_cost) + "," + str(accuracy) + "," +  str(training_time) + "\n")
        fp.close()
