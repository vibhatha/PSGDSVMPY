...
for dataset, feature, split in zip(datasets, features, splits):
    ...
    for gm, ba, br in zip(gamma_momentum, b_adam, b_rmsprop):
        for repition in repetitions:
            now = datetime.datetime.now()
            prefix = str(now.date()) + "_" + str(now.time())
            ########################################LOGS###########################################
            ## SGD Logs
            log_file_sgd = "logs/benchmark/epochlog/" + socket.gethostname() + "_" + exp_name + "_" + prefix + "_" + str(
                repition) + "_sgd_epoch_results.txt"
            init_weight_file_sgd = "weights/benchmark/" + socket.gethostname() + "_" + exp_name + "_" + prefix + "_" + str(
                repition) + "_init_sgd_weights.txt"
            final_weight_file_sgd = "weights/benchmark/" + socket.gethostname() + "_" + exp_name + "_" + prefix + "_" + str(
                repition) + "_final_sgd_weights.txt"
            ## Momentum Logs
            #######################################################################################
            ...
            ################ SGD #################
            w_final_sgd, epochs_eff_sgd, initial_cost_sgd, final_cost_sgd, overall_training_time_sgd = bmsvmsgd.train_benchmark_light(
                x_train=x_training, y_train=y_training, x_test=x_testing, y_test=y_testing, w_init=w_init,
                log_file=log_file_sgd, tolerance=tolerance, indices_init=indices_init)
            ####################################
            ...
            w_final_mom, epochs_eff_mom, initial_cost_mom, final_cost_mom, overall_training_time_mom = bmsvmsgdmomentum.train_benchmark_light(
                x_train=x_training, y_train=y_training, x_test=x_testing, y_test=y_testing, w_init=w_init,
                log_file=log_file_mom, tolerance=tolerance, indices_init=indices_init, gamma=gm)
            ...
            w_final_rmsprop, epochs_eff_rmsprop, initial_cost_rmsprop, final_cost_rmsprop, overall_training_time_rmsprop = bmsvmsgdrmsprop.train_benchmark_light(
                x_train=x_training, y_train=y_training, x_test=x_testing, y_test=y_testing, w_init=w_init,
                log_file=log_file_rmsprop, tolerance=tolerance, indices_init=indices_init, beta=br)
            ...
            w_final_adam, epochs_eff_adam, initial_cost_adam, final_cost_adam, overall_training_time_adam = bmsvmsgdadam.train_benchmark_light(
                x_train=x_training, y_train=y_training, x_test=x_testing, y_test=y_testing, w_init=w_init,
                log_file=log_file_adam, tolerance=tolerance, indices_init=indices_init, beta1=ba[0], beta2=ba[1])
            ...
            ## stats
            bmsvmsgd.advanced_stats(effective_epochs=epochs_eff_sgd, accuracy=overall_accuracy_sgd,
                                    training_time=overall_training_time_sgd, initial_cost=initial_cost_sgd,
                                    final_cost=final_cost_sgd)
