...
class BenchmarkSgdSVM:
    ...
    def train_benchmark_light(self, w_init=[], x_train=[], y_train=[], x_test=[], y_test=[], log_file='', tolerance=0.001, indices_init=[], alpha=0.01):
        ...
        if self.auto:
            Print.Print.console('Auto Alpha Mode')
            self.w, self.effective_epochs, self.cost, self.io_time, self.initial_cost = self.svm.train_sgd_light_no_coff(X=x_train, y=y_train, X_test=x_test, y_test=y_test, alpha=alpha,
                                                                                                               epochs=self.epochs, w_init=w_init, log_file=log_file, tolerance=tolerance,
                                                                                                                 indices_init=indices_init)
        if not self.auto:
            Print.Print.console('Manual Alpha Mode')
            self.w, self.effective_epochs, self.cost, self.io_time, self.initial_cost = self.svm.train_sgd_manual_light(X=x_train, y=y_train, alpha=alpha,
    ...
    def stats(self):
        ...
        fp = open("logs/" + socket.gethostname()+"_"   + self.exp_name + "_sgd_results.txt", "a")
    ...
    def advanced_stats(self, effective_epochs=0, accuracy=0, training_time=0, initial_cost=0, final_cost=0):
        Print.Print.result1(self.exp_name + "SGD Accuracy : " + str(self.acc) + "%")
        Print.Print.result1(self.exp_name + "SGD Time : " + str(self.training_time) + " s")
        fp = open("logs/benchmark/summary/" + socket.gethostname() + "_" + self.exp_name + "_sgd_results.txt", "a")
        # fp.write("alpha : " + str(self.alpha) + ", epochs : " + str(self.epochs) + ", accuracy : " + str(self.acc) + "%" + ", time : " + str(self.training_time) + " s\n")
