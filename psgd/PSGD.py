...
def stats(exp_name='', acc=0, time=0, world_size=1, beta1=0.93, beta2=0.99, batch_size=10, epochs=10):
    Print.Print.result1(
        exp_name + " Parallel SGD SVM Accuracy : " + str(acc) + "%" + ", " + str(time) + "Epochs : " + str(epochs))

    fp = open("logs/" + socket.gethostname() + "_" + exp_name + "_batch_size_" + str(batch_size) + "_cores_" + str(
...
for beta1 in beta1_range:
    for beta2 in beta2_range:
        ...
        if (rank == 0 and isComplete):
            ...
            print("Acc : ", acc)
            stats(exp_name=DATA_SET, acc=acc, time=exp_time, world_size=world_size, beta1=beta1, beta2=beta2, batch_size=-1, epochs=T)
...
#             print(M,len(x_all), len(y_all), x_all[0].shape, y_all[0].shape)
#             for i in range(0, M):
#                 print(i)
#                 X = x_all[i]
#                 y = y_all[i]
