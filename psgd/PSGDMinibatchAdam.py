...
for dataset, features, split in zip(datasets, n_features, splits):
    ...
    for repitition in np.arange(0,1,1):
        n = len(X)
        ...
        print(
        "Total Size, Data Per Machine, Index Per Machine, Batch Size ", n, data_per_machine, len(index_set_of_machine),
        batch_size)
            ...
            for beta2 in beta2_range:
                ...
                for epoch in range(1, epochs):
                    ...
                    for index in indices:
                        ...
                        if (condition < 1):
                            gradient = alpha * (
                            -(X[index] * y[index]) + (coefficient * w))  # gradient accumilation issue must be checked
                        else:
